<x-layout>
                ...
                <div>
                    ...
                    <div class="flex flex-col p-[2rem] w-full">
                        ...
                        <div class="bg-white rounded-md shadow-lg px-6 py-4 w-full mx-auto">
                            <h1 class="font-bold text-2xl mb-4 text-center">Data Analytics of {{auth()->user()->type === 'school' ? $schoolData->name : $user->entity->name}} </h1>
                            <div>
    ...
    <script>
        ...
        if (chartData.length === 0) {
            ...
            `<p class="text-center font-bold text-2xl">No data available</p>`;
        }
        ...
        if (category[0] === 'temperature' || category[0] === 'heart_rate' || category[0] === 'pulse_rate' || category[0] ===
            'respiratory_rate') {
            label = 'Temperature';
        ...
        } else if (category[0] === 'deworming' || category[0] === 'iron_supplementation' || category[0] ===
            'immunization') {
            label = category[0] === 'deworming' ? 'Deworming' : category[0] === 'iron_supplementation' ?
                'Iron Supplementation' : 'Immunization';
            let summary = document.querySelector('.summary');
            summary.innerHTML =
                `<p>Total student that has <span class="font-bold">${category[0] === 'iron_supplementation' ? 'Iron Supplmentation': category[0] === 'deworming' ? 'Deworming': category[0] === 'immunization' ? 'Immunization':''}</span>: ${yesCount}</p>
        <p>Total student that has <span class="font-bold">No ${category[0] === 'iron_supplementation' ? 'Iron Supplmentation': category[0] === 'deworming' ? 'Deworming': category[0] === 'immunization' ? 'Immunization':''}</span>: ${noCount}</p>`;
            ...
            yAxisConfig = {
                beginAtZero: true,
                min: 0,
                max: Math.max(normal_weight, wasted_underweight, severely_wasted_underweight, obese,
                overweight), // Ensure the y-axis max accommodates the counts
                ticks: {
                    stepSize: 1
                }
            };
            ...
            let summary = document.querySelector('.summary');
            summary.innerHTML =
                `<p>Total student that passed <span class="font-bold">${category[0] === 'vision_screening' ? 'Vision Screening': 'Auditory Screening'}</span>: ${passedCount}</p>
            ...
            yAxisConfig = {
                beginAtZero: true,
                min: 0,
                max: Math.max(normalCount, rednessCount, whiteSpotsCount, impetigoCount, skinLessionCount,
                    itchinessCount, acneCount), // Ensure the y-axis max accommodates the counts
                ticks: {
                    stepSize: 1
                }
            };
            ...
            // Prepare the data for the bar chart
            chartValues = [normalCount, rednessCount, whiteSpotsCount, impetigoCount, skinLessionCount, itchinessCount,
                acneCount
            ];
            ...
            const reportChart = new Chart(ctx, {
                type: 'bar', // Bar chart for deworming or iron supplementation
                data: {
                    labels: ['Normal', 'Redness of Skin', 'White Spots', 'Impetigo/Boil', 'Bruises/Injuries',
                        'Skin Lession', 'Itchiness', 'Acne/Pimple'
                    ],
                    datasets: [{
                        label: label,
                        data: chartValues, // Use the counts of Yes and No
            ...
            let summary = document.querySelector('.summary');
            summary.innerHTML =
                `<p>Total student that has <span class="font-bold">Normal</span>: ${normalCount}</p>
                            <p>Total student that has <span class="font-bold">Itchiness</span>: ${itchinessCount}</p>
            ...
            yAxisConfig = {
                beginAtZero: true,
                min: 0,
                max: Math.max(normalCount, styeCount, eyeRedness, ocularCount, paleCount, eyeDischargeCount,
                    mattedCount), // Ensure the y-axis max accommodates the counts
                ticks: {
                    stepSize: 1
                }
            };
            ...
            const reportChart = new Chart(ctx, {
                type: 'bar', // Bar chart for deworming or iron supplementation
                data: {
                    labels: ['Normal', 'Stye', 'Eye Redness', 'Ocular Misallignment', 'Pale Conjunctiva',
                        'Eye Discharge', 'Matted Eyelashes'
                    ],
                    datasets: [{
                        label: label,
                        data: chartValues, // Use the counts of Yes and No
            ...
            yAxisConfig = {
                beginAtZero: true,
                min: 0,
                max: Math.max(normalCount, earDischargeCount,
                impactedCerumenCount), // Ensure the y-axis max accommodates the counts
                ticks: {
                    stepSize: 1
                }
            };
            ...
            yAxisConfig = {
                beginAtZero: true,
                min: 0,
                max: Math.max(normalCount, mucusDischargeCount,
                noseBleedingCount), // Ensure the y-axis max accommodates the counts
                ticks: {
                    stepSize: 1
                }
            };
            ...
            yAxisConfig = {
                beginAtZero: true,
                min: 0,
                max: Math.max(normalCount, enlargeTonsilCount, presenceLessionCount, inflamedCount, enlargeCount,
                    otherCount), // Ensure the y-axis max accommodates the counts
                ticks: {
                    stepSize: 1
                }
            };
            ...
            const reportChart = new Chart(ctx, {
                type: 'bar', // Bar chart for deworming or iron supplementation
                data: {
                    labels: ['Normal', 'Enlarge Tonsils', 'Presence of Lesions', 'Inflamed Pharynx',
                        'Enlarge LymphNodes', 'Others'
                    ],
                    datasets: [{
                        label: label,
                        data: chartValues, // Use the counts of Yes and No
            ...
            yAxisConfig = {
                beginAtZero: true,
                min: 0,
                max: Math.max(normalCount, ralesCount, wheezeCount,
                otherCount), // Ensure the y-axis max accommodates the counts
                ticks: {
                    stepSize: 1
                }
            ...
            yAxisConfig = {
                beginAtZero: true,
                min: 0,
                max: Math.max(normalCount, mumurCount, irregularHeartCount,
                otherCount), // Ensure the y-axis max accommodates the counts
                ticks: {
                    stepSize: 1
                }
            ...
            yAxisConfig = {
                beginAtZero: true,
                min: 0,
                max: Math.max(normalCount, distendedCount, tendernessCount, dysmenorrheaCount,
                otherCount), // Ensure the y-axis max accommodates the counts
                ticks: {
                    stepSize: 1
                }
